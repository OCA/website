...
class LazyLoadTest(SavepointCase):
    ...
    def setUpClass(cls):
        ...
        arch_3 = (
            '<t t-name="test">'
            "&lt;!DOCTYPE html&gt;<html><body><span>content not wrapped"
            "</span></body></html></t>"
        )
        ...
        arch_4 = (
            '<t t-name="test">'
            "&lt;!DOCTYPE html&gt;<html><body><main><span>Teléfono, means phone"
            "</span></main></body></html></t>"
        )
    ...
    def test_normal_render(self):
        ...
        ui_view = (
            self.env["ir.ui.view"]
            .with_user(public_user_id)
            .with_context(website_id=self.website_id)
        )
    ...
    def test_publisher_render(self):
        ...
        ui_view = self.env["ir.ui.view"].with_context(website_id=self.website_id)
        res = etree.HTML(ui_view.render_template(self.view_1.id))
        imgs = res.xpath("//main//img")
    ...
    def test_no_wrap_content(self):
        ...
        arch = (
            "<!DOCTYPE html>\n<html><body><span>content not wrapped"
            "</span></body></html>"
        )
    ...
    def test_encoding_render(self):
        ...
        arch = (
            "<!DOCTYPE html>\n<html><body><main><span>Teléfono, means phone"
            "</span></main></body></html>"
        )
        self.assertEqual(res, arch)
        robots = self.env.ref("website.robots").render()
        self.assertNotIn(
            "<html>", robots.decode("UTF-8"), "Robots must not be wrapped into html DOM"
        )
