...
class WebsiteProductPage(http.Controller):
    ...
    def products_detail(self, product=False, **post):
        if product and (
            product.website_product_published or self._is_website_publisher(request)
        ):
            values = {
                "main_object": product,
                "product": product,
    ...
    @http.route(["/catalog/publish"], type="json", auth="public", website=True)
    ...
    def publish(self, id, object):  # noqa
        ...
        if "website_product_published" in Model._fields:
            values["website_product_published"] = not record.website_product_published
        record.write(values)
