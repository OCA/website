...
class VersioningController(Website):
    ...
    @http.route('/website_version_ce/change_version', type='json', auth="user", website=True)
    def change_version(self, version_id):
        request.session['version_id'] = version_id
        return version_id

    @http.route('/website_version_ce/create_version', type='json', auth="user", website=True)
    def create_version(self, name, version_id=None):
        if not name:
            name = datetime.datetime.now().strftime(DEFAULT_SERVER_DATETIME_FORMAT)
        new_version = request.env['website_version_ce.version'].create({'name': name, 'website_id': request.website.id})
        if version_id:
            request.env['website_version_ce.version'].browse(version_id).copy_version(new_version.id)
        request.session['version_id'] = new_version.id
        return new_version.id

    @http.route('/website_version_ce/delete_version', type='json', auth="user", website=True)
    ...
    @http.route('/website_version_ce/check_version', type='json', auth="user", website=True)
    ...
    @http.route('/website_version_ce/all_versions', type='json', auth="public", website=True)
    ...
    @http.route('/website_version_ce/has_experiments', type='json', auth="user", website=True)
    def has_experiments(self, view_id):
        v = request.env['ir.ui.view'].browse(view_id)
        website_id = request.context.get('website_id')
        return bool(request.env["website_version_ce.experiment.version"].search([
            ('version_id.view_ids.key', '=', v.key),
            ('experiment_id.website_id.id', '=', website_id)
        ], limit=1))

    @http.route('/website_version_ce/publish_version', type='json', auth="user", website=True)
    def publish_version(self, version_id, save_master, copy_master_name):
        request.session['version_id'] = 0
        return request.env['website_version_ce.version'].browse(version_id).publish_version(save_master, copy_master_name)

    @http.route('/website_version_ce/diff_version', type='json', auth="user", website=True)
    ...
    @http.route('/website_version_ce/set_google_access', type='json', auth="user", website=True)
    ...
    @http.route('/website_version_ce/all_versions_all_goals', type='json', auth="user", website=True)
    ...
    @http.route('/website_version_ce/launch_experiment', type='json', auth="user", website=True)
    def launch_experiment(self, name, version_ids, goal_id):
        existing_experiment = request.env['website_version_ce.experiment'].check_no_overlap(version_ids)
