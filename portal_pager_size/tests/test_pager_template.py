...
class TestPagerTemplate(TestPortalPagerSizeCommon):
    ...
    def test_selector_rendered_with_options(self):
        html = self._render_pager(
            {
                "portal_pager_size_options": [10, 20, 40, 80, 100],
                "portal_pager_size": 20,
            }
        )
        self.assertIn("o_portal_pager_size", html)
    ...
    def test_selector_not_rendered_without_options(self):
        ...
        html = self._render_pager()
        self.assertNotIn("o_portal_pager_size", html)
