...
imp_requests = "odoo.addons.portal_recaptcha.models.portal_mixin.requests"


class TestRecaptcha(TransactionCase):
    def setUp(self):
        super().setUp()
        self.portal_model = self.env["portal.mixin"]
        self.env["ir.config_parameter"].sudo().set_param(
            "portal_recaptcha.recaptcha_enabled", True
        )
        self.env["ir.config_parameter"].sudo().set_param(
            "portal_recaptcha.recaptcha_key_site", "test-site"
        )
        self.env["ir.config_parameter"].sudo().set_param(
            "portal_recaptcha.recaptcha_key_secret", "test-secret"
        )
    ...
    def test_captcha_http_request(self, requests_mock):
        ...
        try:
            self.portal_model.is_captcha_valid(
    ...
    def test_captcha_valid(self, requests_mock):
        ...
        result, error_msg = self.portal_model.is_captcha_valid(
            {"g-recaptcha-response": "dummy_response"}
        )
