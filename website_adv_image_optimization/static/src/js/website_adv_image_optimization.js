...
odoo.define('website_adv_image_optimization', function(require) {
    ...
    var AdvancedOptimizationDialog = Dialog.extend({
        ...
        init: function(parent, attachment_id, attachment_src) {
            ...
            this._super(parent, {
                ...
                '$content': $(qweb.render('website_adv_image_optimization.AdvancedOptimizationDialog')),
                'buttons': [
                    { text: _t("Apply"), classes: "btn-primary", click: this.apply },
        ...
        remain_ratio: function(e) {
            ...
            if($target.attr('name') === 'width') {
                this.$('input[name="height"]').val(Math.round($target.val() / this.ratio));
            } else {
                this.$('input[name="width"]').val(Math.round($target.val() * this.ratio));
            }
        }
    });
    ...
    MediaDialog.include({
        xmlDependencies: MediaDialog.prototype.xmlDependencies.concat(
            ['/website_adv_image_optimization/static/src/xml/website_adv_image_optimization.xml']
        ),
        ...
        optimize_image: function(e) {
            var self = this;
            var attachment_id = parseInt($(e.target).attr('data-id'));
            var record = self.imageDialog.records.filter(function(r) { return r.id === attachment_id; });
            if (record.length === 0 || record[0].type === 'url') {
                return;
            }

            var dialog = new AdvancedOptimizationDialog(this, attachment_id, record[0].src);
            dialog.open();
            dialog.applied.done(function() {
                dialog.close();
                self.imageDialog.fetch_existing().then(function() {
                    var record = self.imageDialog.records.filter(function(r) { return r.id === attachment_id; });
                    self.imageDialog.set_image(record[0]);
                });
            });
        }
    });
